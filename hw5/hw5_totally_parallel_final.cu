...
void yourHisto(const unsigned int* const input, //INPUT
  ...
  const unsigned short lo = threadIdx.x;
  ...
  __shared__ unsigned short cidx[2 * xlength];
  __shared__ unsigned short addresses[xlength];
  __shared__ unsigned short localHist[xlength];
  __shared__ unsigned short N;    

  vals[lo] = input[idx];
  localHist[threadIdx.x] = 0; 
  addresses[threadIdx.x] = 0;
  __syncthreads();

  ends[lo] = ( lo > 0 ) ? vals[lo] != vals[lo-1] : 1;
  __syncthreads();

  cidx[lo] = ( lo > 0 ) ? ends[lo-1] : 0;
  __syncthreads();
   ...
   for ( int offset = 1; offset < blockDim.x; offset *= 2) {
       ...
       if ( offset <= threadIdx.x ) // where 'original' input is.
          cidx[lo + next_level] = cidx[lo + level] + cidx[lo - offset + level];
       else
          cidx[lo + next_level] = cidx[lo + level]; //copy over elements with idx < offset
       j++;
       __syncthreads();
   }

   // j % 2 == 0 so we don't need to move scanned indexes to lower half.

   if ( ends[lo] ) addresses[cidx[lo]] = threadIdx.x;
   if ( lo == blockDim.x-1 ) N = ends[lo] + cidx[lo] - 1; // length of compacted array
   __syncthreads();

   if ( ends[lo] )
      localHist[vals[lo]] = (( cidx[lo] < N ) ? addresses[cidx[lo]+1] : blockDim.x) - lo;
   __syncthreads();
  ...
  void computeHistogram(const unsigned int* const d_vals, //INPUT
    ...
    unsigned int *d_sums, * d_in, *d_out;
